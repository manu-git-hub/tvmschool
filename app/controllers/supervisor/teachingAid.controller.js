...
exports.addTeachingAids = async (req, res) => {
    try {
      ...
      // Check if any teaching aid exceeds 300 characters
      ...
      // Prepare bulk data
      const teachingAidsData = teachingAids.map((text) => ({
        teacherId,
        teachingAidText: text,
      }));
  
      // Save all teaching aids
      const newTeachingAids = await TeachingAid.bulkCreate(teachingAidsData);
